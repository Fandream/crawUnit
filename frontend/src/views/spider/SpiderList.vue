<template>
  <div class="app-container">
        ...
        <el-tab-pane name="customized" :label="'自定义'">
          <el-form :model="spiderForm" ref="addCustomizedForm" inline-message label-width="120px">
            <el-form-item :label="'爬虫名称'" prop="name" required>
              <el-input id="spider-name" v-model="spiderForm.name" :placeholder="'爬虫名称'"/>
            </el-form-item>
            <el-form-item :label="'显示名称'" prop="display_name" required>
              <el-input id="display-name" v-model="spiderForm.display_name" :placeholder="'显示名称'"/>
            </el-form-item>
            ...
            <el-form-item :label="'上传Zip文件'" label-width="120px" name="site">
              <el-upload
          ...
          >
            ...
            <p>{{'或者，您也可以点击"上传"按钮并上传一个包含爬虫项目的 zip 文件。'}}</p>
            <p>
              <i class="fa fa-exclamation-triangle"></i> {{'注意: 上传 zip 文件时，请从 根目录 下开始压缩爬虫文件。'}}
            </p>
          ...
          <div class="actions">
            <el-button size="small" type="primary" @click="onAddCustomized">{{'添加'}}</el-button>
          </div>
        </el-tab-pane>
        <!-- configurable -->
      </el-tabs>
    ...
    <el-dialog
      ...
      :title="`${'最近任务'} (${'爬虫'}: ${activeSpider ? activeSpider.name : ''})`"
      width="920px"
    >
      ...
      >
        ...
        >
          <template slot-scope="scope">
            <el-tag type="info" v-if="scope.row.status === 'offline'">{{'离线'}}</el-tag>
            <el-tag type="success" v-else-if="scope.row.status === 'online'">{{'在线'}}</el-tag>
      ...
      <template slot="footer">
        <el-button type="primary" size="small" @click="isRunningTasksDialogVisible = false">{{'确定'}}</el-button>
      </template>
    </el-dialog>
    <!--./running tasks dialog-->
    ...
    <el-card style="border-radius: 0">
      ...
      <el-tabs v-model="filter.type" @tab-click="onClickTab" class="tabs">
        ...
        <el-tab-pane :label="'自定义'" name="customized" class="customized"></el-tab-pane>
        <el-tab-pane :label="'长任务'" name="long-task" class="long-task"></el-tab-pane>
      </el-tabs>
      <!--./tabs-->
      ...
      >
        ...
        <el-table-column :label="'操作'" align="left" fixed="right" min-width="220px">
          <template slot-scope="scope">
            ...
            <el-tooltip v-if="!isShowRun(scope.row)" :content="'No command line'" placement="top">
              <el-button
                disabled
...
export default {
  ...
  computed: {
    ...
    columns () {
      const columns = []
      columns.push({ name: 'display_name', label: '名称', width: '160', align: 'left', sortable: true })
      columns.push({ name: 'type', label: '爬虫类型', width: '120', sortable: true })
      columns.push({ name: 'is_long_task', label: '是否为长任务', width: '80' })
