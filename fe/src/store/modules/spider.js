...
const actions = {
  ...
  getTaskList({ state, commit }, id) {
    return request.get(`/spiders/${id}/tasks`).then(response => {
      commit(
        "task/SET_TASK_LIST",
        response.data.data
          ? response.data.data
  ...
  getSpiderStats({ state, commit }) {
    return request
      .get(`/spiders/${state.spiderForm._id}/stats`)
      .then(response => {
  ...
  getPreviewCrawlData({ state, commit }) {
    return request
      .post(`/spiders/${state.spiderForm._id}/preview_crawl`)
      .then(response => {
  ...
  postConfigSpiderConfig({ state }) {
    return request.post(
      `/config_spiders/${state.spiderForm._id}/config`,
