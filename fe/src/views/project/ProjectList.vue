  ...
  <div class="app-container">
    ...
    <div class="content">
      ...
      <ul v-else class="list">
        <li
          class="item"
          v-for="item in projectList.filter(
            d => d._id !== '000000000000000000000000'
          )"
          :key="item._id"
          @click="onView(item)"
        >
          <el-card class="item-card">
            <i
              v-if="!isNoProject(item)"
              class="btn-edit fa fa-edit"
              @click="onEdit(item)"
            ></i>
            <i
              v-if="!isNoProject(item)"
              class="btn-close fa fa-trash-o"
              @click="onRemove(item)"
            ></i>
            <el-row>
    ...
    onAddSubmit() {
      this.$refs.projectForm.validate(res => {
        if (res) {
          ...
          if (this.isEdit) {
            this.$request
              .post(`/projects/${this.projectForm._id}`, form)
              .then(response => {
