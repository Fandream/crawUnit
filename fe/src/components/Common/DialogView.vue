...
export default {
  name: 'DialogView',
  computed: {
    ...mapState('spider', ['spiderList', 'spiderForm']),
    ...mapState('node', ['nodeList']),
    ...mapState('dialogView', ['dialogType']),
    type () {
  ...
  methods: {
    ...
    onConfirm () {
      if (this.dialogType === 'nodeDeploy') {
        ...
        this.$store
          .dispatch('spider/deploySpider', {
            id: this.spiderForm._id,
            nodeId: this.activeNode._id
          })
          .then(() => {
            this.$message.success(
              `Spider "${this.spiderForm.name}" has been deployed on node "${this.activeNode._id}" successfully`
            );
          })
          .finally(() => {
            // get spider deploys
        ...
        this.$store
          .dispatch('spider/crawlSpider', this.spiderForm._id)
          .then(() => {
            this.$message.success(
              `Spider "${this.spiderForm.name}" started to run on node "${this.activeNode._id}"`
            );
          })
          .finally(() => {
            // get spider tasks
