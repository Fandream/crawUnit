    ...
    >
      ...
      <el-form label-width="140px" :model="form" ref="form">
        ...
        <el-form-item
          ...
          inline-message
        >
          <el-select v-model="form.scrapy_log_level" :placeholder="'Scrapy 日志等级'">
        ...
        <el-form-item v-if="spiderForm.type === 'customized'" :label="'参数'" prop="param" inline-message>
          <template v-if="spiderForm.is_scrapy && !multiple">
            <el-input v-model="form.param" :placeholder="'参数'" class="param-input"/>
            <el-button type="primary" icon="el-icon-edit" class="param-btn" @click="onOpenParameters"/>
          </template>
          <template v-else>
            <el-input v-model="form.param" :placeholder="'参数'"></el-input>
          </template>
        </el-form-item>
        <el-form-item class="checkbox-wrapper">
          ...
          <div v-if="false">
            ...
            <span style="margin-left: 5px">{{'是否重试（最多 5 次）'}}</span>
          </div>
        </el-form-item>
      </el-form>
